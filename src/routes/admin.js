...
function redirectWithMsg(res, path, msg, ok) {
  const sep = path.includes('?') ? '&' : '?';
  res.redirect(`${path}${sep}msg=${encodeURIComponent(msg)}&ok=${ok ? '1' : '0'}`);
}
  ...
  const rows = products
    .map((p) => {
      ...
      return `<tr>
        ...
        <td class="actions">
          <a href="/admin/edit/${encodeURIComponent(p.id)}">تعديل</a>
          <form method="POST" action="/admin/delete/${encodeURIComponent(p.id)}" onsubmit="return confirm('متأكد تبي تمسح ${p.name}؟');">
  ...
  res.send(
    renderPage({
      ...
      body: `
        ...
        <form method="POST" action="/admin/add-product" class="card">
...
router.post('/add-product', express.urlencoded({ extended: true }), async (req, res) => {
  ...
  if (!id || !name || !price || variants.length === 0) {
    return redirectWithMsg(res, '/admin', 'تأكد إنك عبيت كل الحقول المطلوبة والمتغيرات بالشكل الصحيح', false);
  }
  ...
  redirectWithMsg(res, '/admin', result.success ? 'تمت إضافة المنتج بنجاح' : result.reason || 'صار خطأ', result.success);
});
...
router.get('/edit/:id', async (req, res) => {
  ...
  if (!product) {
    return redirectWithMsg(res, '/admin', 'ما لقيتش هذا المنتج', false);
  }
  ...
  res.send(
    renderPage({
      ...
      activePath: '/admin',
      body: `
        <a href="/admin">&larr; رجوع للقائمة</a>
        <h1>تعديل: ${product.name}</h1>
        <form method="POST" action="/admin/edit/${encodeURIComponent(product.id)}" class="card">
...
router.post('/edit/:id', express.urlencoded({ extended: true }), async (req, res) => {
  ...
  if (!name || !price || variants.length === 0) {
    return redirectWithMsg(res, `/admin/edit/${encodeURIComponent(req.params.id)}`, 'تأكد إنك عبيت كل الحقول والمتغيرات بالشكل الصحيح', false);
  }
  ...
  redirectWithMsg(res, '/admin', result.success ? 'تم حفظ التعديلات' : result.reason || 'صار خطأ', result.success);
});

// ── Delete a product ──
router.post('/delete/:id', async (req, res) => {
  const result = await inventory.deleteProduct(req.params.id);
  redirectWithMsg(res, '/admin', result.success ? 'تم حذف المنتج' : result.reason || 'صار خطأ', result.success);
});
